...
for(int a=0; a< string_length_/2; ++a) {
    ...
    if (string_length_%2) {
        ...
        epsilon_[string_length_/2] = 0.;
    }
    else {
        int number_roots = 0;//false;
        for (int beta=0; beta<all.size();++beta)
            number_roots += all[beta]->size();
...
double NonsymString::stepRapidity (const vector<NonsymRoots*>& all, const int alpha)
{
	...
	for (int a=0; a < string_length_; ++a) {
		...
		for (int beta=0; beta<all.size(); ++beta) {
	        ...
	        const vector<double> other_x = all[beta]->getRealRoots();
            const vector<CVal> other_z = all[beta]->getComplexPairs();
            //scatter.add( sum_re_atan_x2( z, other_x, other_z ) );
		    scatter.add(mul( sum_re_atan( z, other_x, other_z ), 2));
		}
	}
...
/*
    ...
    if (string_length_%2==0)
        sum_kin += xi( lambda_, -delta_[string_length_/2-1]);

    // terms pair up in almost-opposites like so:
	...
	// 2\pi \sum_{a=1}^n J_a + \theta\sub{other}^a = \sum_{a=1}^n \theta\sub{kin}^a
    const double phase_x2 = -sum_kin + (PI*sum_jx2_ + scatter.val()) / big_n_;
...
inline double theta_BT (const double lambda, const int length)
{	return 2.0 * atan( 2.0 * lambda / (1.0*length) ) ; 	}


// calculate a new value for the string centre (lambda), using bethe-takahashi
double NonsymString::stepRapidityBT (const vector<NonsymRoots*>& all, const int alpha)// double& smallest_distance)
{
    const double x = lambda_;
    const int l_j = string_length_;
	double scatter = 0.0;
	for (int beta=0; beta<all.size(); ++beta) {
	    if (beta==alpha) continue;

        const vector<double> other_x = all[beta]->getRapidities();
        const int l_k = all[beta]->stringLength();
	    for (int k=0; k < other_x.size();++k) {
	        const double x_x_k = x - other_x[k];
            if (l_j != l_k)
			    scatter += theta_BT(x_x_k, abs(l_j-l_k));
		    for (int i=1; i< min(l_j, l_k); ++i)
			    scatter += 2.0 * theta_BT(x_x_k, abs(l_j-l_k) + 2*i);
            scatter += theta_BT(x_x_k, l_j+l_k);
        }
    }
    return 0.5*string_length_ * tan( 0.5*(PI*ix2_ + scatter) / big_n_ );
}


// extract sum of Bethe J's from Takahashi I configuration
// NOT USED
const int NonsymString::calculateSum2xJ(const vector<NonsymRoots*>& all, const int alpha)
{
	const int l_j = string_length_;
	sum_jx2_ = ix2_ + big_n_*(l_j-1);
	...
	for (int k=0; k < all.size(); ++k) {
        ...
        for (int beta=0; beta<lambda_k.size(); ++beta) {
            sum_jx2_ -= isgn(lambda_ - lambda_k[beta]) * ( l_j*l_k - 2*min(l_j, l_k) + (l_j==l_k) );
	    }
	}

	while (sum_jx2_ > big_n_) sum_jx2_ -= 2*big_n_;
	while (sum_jx2_ <= -big_n_) sum_jx2_ += 2*big_n_;

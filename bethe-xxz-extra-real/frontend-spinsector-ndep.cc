...
// 	REAL precision = 1.5* 1e-24;
	...
	cerr<<"N_min   N_max  M "<<endl;
	cin >> number_sites_min >>  number_sites_max >> number_down_left;
	policy.precision *= number_down_left;

	stringstream file_name;
	file_name << "N"<<number_sites_min<< "--"<<number_sites_max<<"_M"<<number_down_left<<".count";
	const char* outfile_name = file_name.str().c_str();
	ofstream outfile (outfile_name);

	for (int number_sites=max(2*number_down_left, number_sites_min); number_sites<number_sites_max; number_sites+=2)
	{
